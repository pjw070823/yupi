...
async def on_message(msg):
    ...
    #     else:
    #         model = random.choice(["google/gemini-2.5-flash-lite", "google/gemini-2.5-flash", "google/gemini-flash-1.5"])
    #         try:
    #             response = await client.chat.completions.create(
    #                 model=model,
    #                 temperature=1,
    #                 messages=[
    #                     {"role": "system", "content": "너는 디스코드 챗봇인 유피이야. 너는 여자아이야. 사용자가 질문하거나 말하는 내용에 귀여운 말투로 대답해. 질문한 내용은 성실히 대답해.사용자에게 역질문은 하지마. 사용자가 골라달라하는거 같으면 아무거나 골라. 이모지는 사용하지 마. 어머 라는 감탄사는 쓰지 마. 대화형 챗봇인만큼 길지 않게 대답해."},
    #                     {"role": "user", "content": "다음 줄부터가 사용자의 입력이야.\n" + text}
    #                 ]
    #             )
    #             await msg.channel.send(response.choices[0].message.content)
    #         except Exception as exc:
    #             await msg.channel.send(f"API 오류: {exc}")
        ...
        if text == '' and not msg.attachments:
            ...
            for attachment in msg.attachments:
                ...
                if content_type.startswith("image/"):
                    try:
                        ...
                        image_parts.append((content_type, image_bytes))
                    except Exception:
                        pass
            ...
            if image_parts:
                ...
                user_content = [
                    ...
                    *[{
                        "type": "image_url",
                        "image_url": {"url": f"data:{ct};base64,{base64.b64encode(ib).decode()}"}
        ...
        if len(text) > 500:
            ...
            try:
                response = await client.chat.completions.create(
                    ...
                    temperature=1,
                    messages=[
                        {"role": "user", "content": "너는 디스코드 채팅 앱의 챗봇인 유피이야. 사용자가 질문하거나 말하는 내용에 길지 않게  대답해. 다음 줄부터가 사용자의 입력이야.\n" + text}
                    ]
                )
                await msg.channel.send(response.choices[0].message.content)
            except Exception as exc:
...
async def calc(interaction, text1: str):
    respond = ''
    baned = False
    ban_list = ['print', 'input', 'eval', 'bot', 'client', 'return', 'break', 'on_message', 'chat', 'calc', 'os', 'sqlite', 'sys']
...
async def chat(interaction, text: str):
    if len(text) > 500:
        ...
        response = await client.chat.completions.create(
            ...
            temperature=2,
            messages=[
                {"role": "user", "content": "너는 디스코드 채팅 앱의 챗봇인 유피이야. 사용자가 질문하거나 말하는 내용을 정확히 대답해. 다음 줄부터가 사용자의 입력이야.\n" + text}
            ]
        )
    ...
    if len(text) > 500:
        ...
        if len(text) > 500:
            ...
            try:
                ...
                await interaction.edit_original_response(content=f"API 오류: {exc}")
                return
