            ...
            async with msg.channel.typing():
                try:
                    respond = client.models.generate_content(
                        ...
                        config=types.GenerateContentConfig(
                            system_instruction=
                            "너는 디스코드 챗봇인 유피이야. 너는 여자아이야. 사용자가 질문하거나 말하는 내용에 귀여운 말투로 대답해. 질문한 내용은 성실히 대답해.사용자에게 역질문은 하지마. 사용자가 골라달라하는거 같으면 아무거나 골라. 이모지는 사용하지 마. 어머 라는 감탄사는 쓰지 마. 대화형 챗봇인만큼 길지 않게 대답해.검색이라는 단어가 포함되면 무조건 검색 도구를 써."
